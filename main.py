...
parser.add_argument(
	'--batch-file',
	type=str,
	required=('-tm' not in sys.argv and '--telegram-channel' not in sys.argv),
...
if all(i is not None for i in args.values()):
	parser.error('Select either (-tm / --telegram-channel) or --batch-file options only.')
...
for channel in req_input:
	...
	if not args['limit_download_to_channel_metadata']:
		...
		# Collect posts
		posts = loop.run_until_complete(get_posts(client, channel_id))
		data = posts.to_dict()
		...
		while len(posts.messages) > 0:
			...
			if posts.messages:
				...
				chats = [i for i in tmp['chats'] if i['id'] not in all_chats]
				...
				users = [i for i in tmp['users'] if i['id'] not in all_users]
